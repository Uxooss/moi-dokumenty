...
(function () {
  ...
  const TRANSLATIONS = {
    uk: {
      ...
      confirmDeleteMsg:
        "Цю дію неможливо відмінити. Документ та всі прикріплені файли будуть видалені назавжди.",
      confirmClearTitle: "Очистити всі дані?",
      confirmClearMsg:
        "Всі документи та налаштування будуть видалені безповоротно.",
      settingsTitle: "Налаштування",
      labelWarnDays: "Попереджати за (днів до закінчення строку)",
      warnDaysNote:
        "Документи, до закінчення яких залишилось менше вказаної кількості днів, позначаються жовтим.",
      ...
      encryptionNote:
        "Шифрує документи за допомогою PIN-коду (AES-GCM). Потребує встановленого PIN.",
      ...
      backupNote:
        "Усі документи зберігаються лише в браузері на цьому пристрої.",
      backupSaveTitle: "Зберегти копію",
      backupSaveDesc:
        "Відкриє вікно вибору місця або завантажить у «Завантаження»",
      ...
      warnDuplicate:
        "Документ із такою назвою вже є. Натисніть «Зберегти» ще раз.",
    ...
    en: {
      ...
      confirmDeleteMsg:
        "This action cannot be undone. The document and all attached files will be permanently deleted.",
      confirmClearTitle: "Clear all data?",
      confirmClearMsg:
        "All documents and settings will be permanently deleted.",
      settingsTitle: "Settings",
      labelWarnDays: "Warn before (days until expiry)",
      warnDaysNote:
        "Documents expiring within this number of days are highlighted in yellow.",
      ...
      encryptionNote:
        "Encrypts documents using PIN code (AES-GCM). Requires a PIN.",
      ...
      backupTitle: "Backup",
      backupNote:
        "All documents are stored only in this browser. Save backups to external storage.",
      ...
      warnDuplicate:
        "A document with this name already exists. Press Save again to confirm.",
  ...
  async function getStore(n, m) {
    const db = await openDB();
    return db.transaction(n, m || "readonly").objectStore(n);
  }
  async function dbGetAll(n) {
    const s = await getStore(n);
  ...
  async function dbPut(n, v) {
    const s = await getStore(n, "readwrite");
    return new Promise((res, rej) => {
      const r = s.put(v);
      r.onsuccess = () => res(r.result);
      r.onerror = () => rej(r.error);
  ...
  function cacheDOMRefs() {
    ...
    scopeActiveBtn = $("scopeActiveBtn");
    scopeArchiveBtn = $("scopeArchiveBtn");
    scopeActiveCount = $("scopeActiveCount");
    scopeArchiveCount = $("scopeArchiveCount");
    editModal = $("editModal");
    docForm = $("docForm");
    modalTitle = $("modalTitle");
    nameUaInput = $("nameUaInput");
    nameEnInput = $("nameEnInput");
    ...
    warnDaysInput = $("warnDaysInput");
    saveSettingsBtn = $("saveSettingsBtn");
    clearAllBtn = $("clearAllBtn");
    pinStatusNote = $("pinStatusNote");
    setPinBtn = $("setPinBtn");
    ...
    dashTotal = $("dashTotal");
    dashValidCount = $("dashValidCount");
    dashSoonCount = $("dashSoonCount");
    dashExpiredCount = $("dashExpiredCount");
    dashNoneCount = $("dashNoneCount");
  ...
  function getPreviewInfo(doc, large) {
    ...
    if (c.type && c.type.startsWith("image/"))
      return { kind: "img", url: blobURL(c.blob) };
    if (c.type === "application/pdf")
      return large
        ? { kind: "pdf-live", url: blobURL(c.blob) }
        : { kind: "pdf-icon" };
    return { kind: "none" };
  }
  ...
  function getPinAttempts() {
    return parseInt(sessionStorage.getItem("pinAttempts") || "0", 10);
  }
  function setPinAttempts(n) {
    sessionStorage.setItem("pinAttempts", String(n));
  }
  function getPinLockUntil() {
    return parseInt(sessionStorage.getItem("pinLockUntil") || "0", 10);
  }
  function setPinLockUntil(t) {
    sessionStorage.setItem("pinLockUntil", String(t));
  }
  ...
  function bindPINEvents() {
    pinOverlay.addEventListener("click", async (e) => {
      ...
      if (!k) return;
      if (k === "clear") {
        if (pinBuffer.length) pinBuffer = pinBuffer.slice(0, -1);
        updatePinDots();
        return;
      }
      if (k === "ok") {
    ...
    removePinBtn.addEventListener("click", async () => {
      const ok = await showConfirm(
        t("confirmRemovePinTitle"),
        t("confirmRemovePinMsg"),
        t("removePin"),
        true,
      );
      if (!ok) return;
  ...
  async function loadSettings() {
    const rows = await dbGetAll("settings");
    state.settings.warnDays = (
      rows.find((r) => r.key === "warnDays") || { value: 30 }
    ).value;
    state.settings.autoLockMinutes = (
      rows.find((r) => r.key === "autoLockMinutes") || { value: 0 }
    ).value;
  ...
  async function maybeSeed() {
    ...
    for (const sd of seed) {
      ...
      for (const f of sd.files) {
        ...
        const thumb = f.thumbB64
          ? await dataURLToBlob(`data:image/jpeg;base64,${f.thumbB64}`)
          : null;
    ...
    await dbPut("settings", { key: "seeded_v1", value: true });
    showToast(
      `Додано ${seed.length} ${pluralUk(seed.length, [t("doc1"), t("doc24"), t("doc5")])}`,
      "success",
    );
  }
  ...
  function renderDashboard() {
    ...
    const upcoming = active
      .filter((d) => {
        ...
        return (
          s.key === "soon" ||
          (s.key === "valid" && s.daysLeft !== null && s.daysLeft <= 90)
        );
      })
      .sort((a, b) => daysBetween(a.expiryDate) - daysBetween(b.expiryDate))
      .slice(0, 8);

    dashTimeline.innerHTML = upcoming.length
      ? upcoming
          .map((d) => {
            const s = getDocStatus(d);
        ...
        <span class="dtl-days">${s.daysLeft} ${t("daysLeft")}</span>
          ...
          .join("")
      : `<p class="muted-note">${t("emptyText")}</p>`;
  }
  ...
  function getFilteredDocs() {
    let list = state.docs.filter(
      (d) => !!d.archived === (state.filters.scope === "archive"),
    );
    ...
    if (state.filters.tags.size)
      list = list.filter((d) =>
        (d.tags || []).some((tg) => state.filters.tags.has(tg)),
      );
    ...
    if (state.filters.search.trim()) {
      ...
      list = list.filter(
        (d) =>
          ...
          fmtDate(d.issueDate).includes(q) ||
          fmtDate(d.expiryDate).includes(q) ||
          (d.issueDate || "").includes(q) ||
          (d.expiryDate || "").includes(q) ||
    ...
    list.sort((a, b) => {
      if (state.sort === "expiry-asc") {
        const da =
          a.noExpiry || !a.expiryDate ? Infinity : daysBetween(a.expiryDate);
        const db2 =
          b.noExpiry || !b.expiryDate ? Infinity : daysBetween(b.expiryDate);
        return da - db2;
      }
  ...
  function thumbHtml(doc, large) {
    ...
    if (info.kind === "pdf-live")
      return `<iframe src="${info.url}#toolbar=0&navpanes=0&view=FitH" loading="lazy"></iframe>`;
    if (info.kind === "pdf-icon")
      return `<div class="doc-thumb-placeholder"><span class="ph-icon">PDF</span></div>`;
  ...
  function archCtlHtml(doc) {
    const s = getDocStatus(doc);
    if (doc.archived)
      return `<button class="unarchive-btn" data-unarchive="${doc.id}">${t("fromArchive")}</button>`;
    if (s.key === "expired")
      return `<button class="archive-btn" data-archive="${doc.id}">${t("toArchive")}</button>`;
    return "";
  }
  ...
  function cardRow(doc) {
    const s = getDocStatus(doc),
      info = getPreviewInfo(doc, false);
    const thumb =
      info.kind === "img"
        ? `<img src="${info.url}" alt="">`
        : `<div class="ph-icon">${info.kind === "pdf-icon" ? "PDF" : "—"}</div>`;
    const dateText = doc.expiryDate
    ...
    const archBtn = doc.archived
      ...
      : s.key === "expired"
        ? `<button class="row-archive-btn" data-archive="${doc.id}">${t("toArchive")}</button>`
        : "";
  ...
  function renderSidebar() {
    const active = state.docs.filter((d) => !d.archived),
      arch = state.docs.filter((d) => d.archived);
    ...
    scopeArchiveBtn.classList.toggle(
      "active",
      state.filters.scope === "archive",
    );
    statusFilterGroup.classList.toggle(
      "hidden",
      state.filters.scope === "archive",
    );
    ...
    statusFiltersEl.innerHTML = STATUS_DEFS_KEYS.map(
      (key) =>
  ...
  function renderCards() {
    ...
    if (state.viewMode === "list") {
      ...
      cardsGrid.innerHTML = list.map(cardRow).join("");
    } else if (state.viewMode === "large") {
      cardsGrid.className = "cards-grid mode-large";
      cardsGrid.innerHTML = list.map(cardLarge).join("");
    } else {
      cardsGrid.className = "cards-grid";
      cardsGrid.innerHTML = list.map(cardTile).join("");
    }
  }
  ...
  async function addFiles(fileList) {
    for (const f of Array.from(fileList)) {
      if (!ALLOWED_TYPES.includes(f.type)) {
        showToast(`«${f.name}» ${t("fileSkippedType")}`, "error");
        continue;
      }
      if (f.size > 25 * 1024 * 1024) {
        showToast(`«${f.name}» ${t("fileSkippedSize")}`, "error");
        continue;
      }
  ...
  function renderAttachList() {
    ...
    attachList.innerHTML = state.formFiles
      .map((f, i) => {
        const url = blobURL(f.thumb || f.blob);
        const thumb =
          f.thumb || f.type.startsWith("image/")
  ...
  function bindAttachEvents() {
    ...
    attachList.addEventListener("click", (e) => {
      ...
      if (ci !== null) {
        const x = +ci;
        const [it] = state.formFiles.splice(x, 1);
        state.formFiles.unshift(it);
        renderAttachList();
      } else if (vi !== null) {
        const f = state.formFiles[+vi];
        if (f) window.open(blobURL(f.blob), "_blank");
      } else if (ri !== null) {
        state.formFiles.splice(+ri, 1);
        renderAttachList();
      }
    });
  }
  ...
  function clearDateErrors() {
    ...
    [issueDateInput, expiryDateInput].forEach((i) =>
  ...
  function openEditModal(doc) {
    ...
    formTags = (doc.tags || []).slice();
    renderFormTags();
    clearDateErrors();
    state.formFiles = (doc.files || []).map((f) => ({
      name: f.name,
      type: f.type,
      blob: f.blob,
      thumb: f.thumb || null,
  ...
  function bindEditModalEvents() {
    ...
    docForm.addEventListener("submit", async (e) => {
      ...
      if (!nameUa) {
        showToast("errNameRequired", "error");
        nameUaInput.focus();
        return;
      }
      if (!noExpiryCheckbox.checked && !expiryDateInput.value) {
        showToast("errExpiryRequired", "error");
  ...
  function renderPreviewContent() {
    ...
    previewCounter.textContent =
      files.length > 1 ? `${state.previewFileIndex + 1} / ${files.length}` : "";
    previewPrevBtn.classList.toggle("hidden", state.previewFileIndex === 0);
    previewNextBtn.classList.toggle(
      "hidden",
      state.previewFileIndex >= files.length - 1,
    );
  }
  ...
  function renderPreviewMeta() {
    ...
    if (doc.nameEn)
      html += `<div class="pm-row"><span class="pm-label">${t("labelNameEn")}</span><span>${esc(doc.nameEn)}</span></div>`;
    if (doc.number)
      html += `<div class="pm-row"><span class="pm-label">${t("labelNumber")}</span><span>№ ${esc(doc.number)}</span></div>`;
    if (doc.issueDate)
      html += `<div class="pm-row"><span class="pm-label">${t("labelIssueDate")}</span><span>${fmtDate(doc.issueDate)}</span></div>`;
    ...
    if (doc.tags && doc.tags.length)
      html += `<div class="pm-row"><span class="pm-label">${t("labelTags")}</span><span>${doc.tags.map((tg) => `<span class="tag-pill">${esc(tg)}</span>`).join(" ")}</span></div>`;
    if (doc.notes)
      html += `<div class="pm-row"><span class="pm-label">${t("labelNotes")}</span><span>${esc(doc.notes)}</span></div>`;
    html += `</div>`;
  ...
  function bindPreviewEvents() {
    ...
    previewNextBtn.addEventListener("click", () => {
      ...
      if (state.previewFileIndex < files.length - 1) {
        state.previewFileIndex++;
        renderPreviewContent();
      }
    });
  ...
  function pushUndo(action, data) {
    ...
    undoMessage.textContent =
      action === "delete" ? t("toastDeleted") : t("toastArchived");
  ...
  function bindCoreEvents() {
    ...
    cardsGrid.addEventListener("click", (e) => {
      ...
      if (ub) {
        e.stopPropagation();
        unarchiveDoc(+ub.getAttribute("data-unarchive"));
        return;
      }
    ...
    tagFiltersEl.addEventListener("click", (e) => {
      ...
      state.filters.tags.has(tg)
        ? state.filters.tags.delete(tg)
        : state.filters.tags.add(tg);
      renderAll();
    });
    ...
    scopeArchiveBtn.addEventListener("click", () => {
      state.filters.scope = "archive";
      renderAll();
    });
  ...
  async function exportCalendar() {
    const docs = state.docs.filter(
      (d) => !d.archived && d.expiryDate && !d.noExpiry,
    );
    if (!docs.length) {
      showToast("warnNoCalendarDocs", "warn");
      return;
    }
    const lines = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//DocVault//Certificates//UK",
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH",
      `X-WR-CALNAME:${t("reportTitle")}`,
    ];
    for (const d of docs) {
      lines.push(
        "BEGIN:VEVENT",
    ...
    const content = lines.join("\r\n"),
      ...
      filename = `sertyfikaty-${stamp}.ics`;
    if (window.showSaveFilePicker) {
      try {
        const h = await window.showSaveFilePicker({
          suggestedName: filename,
          types: [
            { description: "iCalendar", accept: { "text/calendar": [".ics"] } },
          ],
        });
        const w = await h.createWritable();
        await w.write(content);
        await w.close();
        showToast("toastCalendarSaved", "success");
        return;
      } catch (e) {
  ...
  function printReport() {
    const list = getFilteredDocs();
    if (!list.length) {
      showToast("warnNoReportDocs", "warn");
      return;
    }
    ...
    if (!w) {
      showToast("errPopupBlocked", "error");
      return;
    }
    ...
    exportBtn.addEventListener("click", async () => {
      try {
        ...
        const jsonStr = JSON.stringify(payload),
          stamp = new Date().toISOString().slice(0, 10),
          filename = `dokumenty-rezervna-kopiya-${stamp}.json`;
        if (window.showSaveFilePicker) {
          try {
            const h = await window.showSaveFilePicker({
              suggestedName: filename,
            ...
            await w.close();
            showToast("toastBackupSaved", "success");
            return;
          } catch (e) {
            if (e && e.name === "AbortError") return;
          }
        ...
        console.error(e);
        showToast("errBackupFailed", "error");
      }
    });
      ...
      try {
        ...
        for (const d of payload.docs) {
          ...
          const existing = state.docs.find(
            (ex) =>
              (ex.nameUa || "").toLowerCase() ===
                (d.nameUa || "").toLowerCase() &&
              (ex.number || "").toLowerCase() ===
          ...
          delete newDoc.id;
          const raw = Array.isArray(d.files)
            ? d.files
            : d.fileBase64
              ? [
                  {
          ...
          for (const f of raw) {
            ...
            const thumb = f.thumbDataUrl
              ? await dataURLToBlob(f.thumbDataUrl)
              : null;
            if (blob) files.push({ name: f.name, type: f.type, blob, thumb });
          }
          newDoc.files = files;
          newDoc.archived = !!d.archived;
          delete newDoc.fileBase64;
          delete newDoc.fileName;
          delete newDoc.fileType;
          delete newDoc.fileData;
        ...
        renderAll();
        showToast(
          `${t("toastImported")}: ${count}${skipped ? ` (${skipped} skipped)` : ""}`,
          "success",
        );
        closeModal(backupModal);
      } catch (e) {
        console.error(e);
        showToast("errBadBackup", "error");
      } finally {
